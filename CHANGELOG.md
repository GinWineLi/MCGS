...
## 2026-06-12 12:51 - 修复服务器评估导入路径

- 更新时间：2026-06-12 12:51 CST
- 更新记录：修复 Hydra 输出目录下运行 ACO/MKP 等评估脚本时无法导入项目 `utils` 包的问题。
- 更新内容：
  - `problem_adapter.py`: 在 eval bootstrap 的 `sys.path` 中加入项目根目录，并保持隔离候选 `gpt.py` 优先于问题目录和项目根目录。
- 验证情况：已运行 `python -c "compile(...)"` 语法检查、bootstrap 导入路径烟测、带 `sys.path.insert(0, "../../../")` 的相对路径烟测，以及 `git diff --check`。
- Git 提交：fix(eval): 修复评估子进程导入项目工具包
